...
const semverConfig: TagsmithConfig = {
  pattern: "v{version}",
  model: { type: "semver" },
...
describe("planNext", () => {
  it("bumps from the latest conforming tag", () => {
    const plan = planNext(semverConfig, model, ["v1.0.0", "v1.2.0"], "patch");
  ...
  it("uses initialVersion when no conforming tag exists", () => {
    const plan = planNext(semverConfig, model, [], "patch");
  ...
  it("ignores non-conforming tags when computing latest", () => {
    const plan = planNext(semverConfig, model, ["garbage", "v2.0.0"], "minor");
    expect(plan.tag).toBe("v2.1.0");
  });

  it("guarantees strict increase via build model", () => {
    const buildConfig: TagsmithConfig = {
    ...
    const plan = planNext(buildConfig, bm, ["build-7"], "auto");
...
describe("validateExplicit", () => {
  it("accepts a strictly greater new version", () => {
    const r = validateExplicit(semverConfig, model, "1.3.0", ["v1.2.0"]);
    expect(r.ok).toBe(true);
    expect(r.errors).toHaveLength(0);
  });

  it("rejects an out-of-order version", () => {
    const r = validateExplicit(semverConfig, model, "1.0.0", ["v1.2.0"]);
  ...
  it("rejects an existing tag", () => {
    const r = validateExplicit(semverConfig, model, "1.2.0", ["v1.2.0"]);
    expect(r.ok).toBe(false);
    expect(r.errors.some((e) => /already exists/.test(e))).toBe(true);
  });

  it("rejects an unparseable version", () => {
    const r = validateExplicit(semverConfig, model, "not-semver", []);
    expect(r.ok).toBe(false);
